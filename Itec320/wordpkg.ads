-- Ada package body gives the implementation for the word abstract
...
-- is used to determine when one word is less than another.
   ...
   function ">=" (X, Y : Word) return Boolean;
   function ">"  (X, Y : Word) return Boolean;
   function charAt(Item : Word; Index : Positive) return Character;
   function lowerCase(Item : Word) return Word;
--   function upperCase(Item : Word) return Word;
